...
export default function Profile() {
    ...
    if (!userInfo) {
        return (
            <div className="flex items-center justify-center h-screen bg-[#1b1c24]">
    ...
    const handleImageChange=async(event)=>{
        ...
        if(file){
            ...
            if(response.status===200 && response.data.image){
                ...
                console.log("setuserinfo",response.data.image)
    ...
    return (
        <div className="bg-[#1b1c24] h-[100vh] items-center justify-center flex-col gap-10">
                ...
                <div className='grid grid-cols-2'>
                    <div className='h-full w-32 md:w-48 relative flex items-center justify-center'
                        ...
                        <Avatar className="h-32 w-32 md:w-48 md:h-48 rounded-full overflow-hidden">
                            {
                                image ? (<AvatarImage src={image} alt="profile" className="object-cover w-full h-full  object-fill bg-black"/>)
                                    : (<div className={`uppercase h-32 w-32 md:w-48 md:h-48 text-5xl border-[1px] flex justify-center items-center rounded-full ${getColors(selectedColor)}`} >
                                        {
                        ...
                        {
                            hovered &&  (
                                <div className=' absolute inset-0 flex items-center justify-center bg-black/50 rounded-full cursor-pointer ring-fuchsia-50' onClick={image ? handleDeleteImage : handleFileInputClick}>
                                    {
                        ...
                        <div className='w-full flex gap-5'>
                            {
                                ...
                                ${selectedColor===index ? "outline outline-white/50 outline-8" : ""}}`} key={index} onClick={()=>setSelectedColor(index)}>
                                </div>
                                ))}
                        </div>
                    </div>
